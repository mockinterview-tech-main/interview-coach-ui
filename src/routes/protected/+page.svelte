                ...
                if (!fu.done) {
                    ...
                    if (resp?.summary_text) {
                        try {
                            ...
                            interviewSummaryStore.set({...resp})
                        }
                    }
                }
                loading = false;
...
{#if $interviewQuestion.question_text === "" && !$interviewSummaryStore}
    <GetStartedCard conversation={conversation} jobInfo={jobInfo} handleJobInfo={handleJobInfo}/>
{:else}
    <ConversationCard conversation={conversation} loading={loading}/>
{/if}

{#if !loading && $interviewQuestion.question_text && !$interviewSummaryStore}
    <RecordAnswerButton testFu={testFu} testLLM={testLLM}/>
{/if}

{#if isDone && $interviewSummaryStore}
