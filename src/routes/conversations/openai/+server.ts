...
export const POST = async ({request}) => {
    ...
    const motivation: ChatCompletionSystemMessageParam = {
        role: 'system',
        content: `You are conducting an interview to hire someone for a role at a company. You are taking on the role of the first person to speak in the given conversation.
    ...
    const rules: ChatCompletionSystemMessageParam = {
        role: 'system',
        content: ` Ask ${questionCount || 1} total questions, one at a time. Based on the candidate's responses ask ${followupCount || 1} followup questions, one at a time, before moving on to the next top-level question.
